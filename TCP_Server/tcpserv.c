...
struct tcp_server_struct {
	...
	struct tcp_pcb *pcb;
	/* pointer on the received/to be transmitted pbuf */
	struct pbuf *p;
};
...
void tcp_server_init(void){
  ...
  if (tcp_server_pcb != NULL)
  {
    ...
    err = tcp_bind(tcp_server_pcb, &myIPADDR, TCP_SERVER_PORT);
 ...
 * @brief
 */
void tcp_log(struct pbuf *p){
    int i;
    printf("Received:tot:%d len:%d type:%d\n", p->tot_len, p->len, p->type_internal);
    //((u8_t *)q->payload)[i]
    for(i=0; i< p->len; i++){
        tcp_buffer[i] = ((uint8_t *)p->payload)[i];
    }
    tcp_buffer[i] = '\0';
...
/**
  * @brief  This function implements the tcp_sent LwIP callback
  *         (called when ACK is received from remote host for sent data)
  * @param  None
  * @retval None
  */
static err_t tcp_server_sent(void *arg, struct tcp_pcb *tpcb, u16_t len)
