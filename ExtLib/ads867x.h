...
typedef union __attribute__((aligned(1), packed)) {
	...
	/* Receive data struct order. */
	struct {
		uint8_t DataWord_HSW_MSB;
		uint8_t DataWord_HSW_LSB;
		uint8_t DataWord_LSW_MSB;		
		uint8_t DataWord_LSW_LSB;		
		
	};
	/* Bits for transmit data struct order. */
	struct {
		/* */
