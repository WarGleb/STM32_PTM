/*
	@brief CRC calculation block.
	Created 03.24.22.
	Author: asw3005.
*/

#include "stm32f4xx_hal.h"
#include "CRCCalc.h"

/* CRC8 table. Calculated using polynomial value 0xD5 (x^8 + x^7 + x^6 + x^4 + x^2 + 1) */
static const uint8_t crc8_table[256] =
{
	
};

/*
	@brief CRC8 calculation.
	@param *_buffer: pointer for the data buffer.
	@param _length: length of the buffer.
*/
uint8_t Soft_CRC8(const uint8_t * _buffer, const uint16_t _length)
{
	register uint16_t iterator;
	register uint8_t crc = 0;

	//if (_length == 0) return 0x00;
	for (iterator = 0; iterator < _length; iterator++)
		crc = crc8_table[crc ^ _buffer[iterator]];
	return crc;
}

/*
	@brief Hardware CRC calculation.
	@param *_buffer: pointer for the data buffer.
	@param _length: length of the buffer.
*/
uint32_t Hard_CRC32ETH(uint8_t *_buffer, uint8_t _length)
{
	//RCC->AHBENR |= RCC_AHBENR_CRCEN;
	CRC->CR = CRC_CR_RESET;
	for (uint8_t i = 0; i < _length; i++)
	{
		CRC->DR = (uint32_t)(*_buffer);
		_buffer++;
		__NOP(); __NOP(); __NOP(); __NOP(); __NOP();
	}
	return CRC->DR;
}



